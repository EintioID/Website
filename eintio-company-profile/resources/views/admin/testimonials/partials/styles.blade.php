{{-- resources/views/admin/testimonials/partials/styles.blade.php --}}
<style>
    ...
    .stat-card { border: 1px solid #E2E8F0; border-radius: 1rem; transition: box-shadow .3s var(--ease-smooth), transform .3s var(--ease-smooth), border-color .3s var(--ease-smooth); }
    .stat-card:hover { box-shadow: 0 18px 36px -18px rgba(15,23,42,0.2); transform: translateY(-3px); border-color: rgba(20,184,166,0.3); }

    .team-table-card { border: 1px solid #E2E8F0; border-radius: 1rem; transition: box-shadow .3s var(--ease-smooth), border-color .3s var(--ease-smooth); }
    ...
    .dropdown-trigger {
        ...
        .dropdown-wrap:has(.dropdown-trigger.is-open) { z-index: 40; }
    ...
    /* ===== Table row ===== */
    .team-row { position: relative; overflow: hidden; transition: background-color .25s var(--ease-smooth); }
    .team-row:hover { background-color: #FBF6EE; }
    .team-row-bar { position: absolute; left: 0; top: 0; height: 100%; width: 0; transition: width .25s var(--ease-smooth); }
    .team-row:hover .team-row-bar { width: 3px; }

    .avatar-circle { transition: transform .3s var(--ease-smooth), box-shadow .3s var(--ease-smooth); }
    .team-row:hover .avatar-circle { transform: scale(1.1) rotate(-2deg); box-shadow: 0 8px 18px -6px rgba(15,23,42,0.3); }
    ...
    /* ===== Rating stars ===== */
    .star-icon { transition: transform .2s var(--ease-smooth); }
    .rating-group:hover .star-icon { transform: scale(1.05); }

    /* ===== Quote card (detail page) ===== */
    .quote-card { position: relative; border-radius: 1rem; transition: box-shadow .3s var(--ease-smooth), transform .3s var(--ease-smooth); }
    .quote-card:hover { box-shadow: 0 18px 36px -18px rgba(15,23,42,0.18); transform: translateY(-2px); }
</style>