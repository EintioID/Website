<x-app-layout>
    ...
    <style>
        ...
        /* FIX: icon di dalam input butuh padding-left lebih besar,
           override shorthand padding di .form-input yang menang
           lawan utility class pl-11 */
        .form-input.pl-11 {
    ...
    <div x-data="settingsPage()" class="space-y-6 px-4 md:px-6 py-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <form method="POST" action="{{ route('admin.settings.preferences') }}" 
                ...
                {{-- BAHASA --}}
                <div>
                    <label for="language-select" class="block text-xs font-bold text-slate-700 mb-3 uppercase tracking-wide">
                        <i class="fa-solid fa-language text-teal-500 mr-1"></i>{{ __('Bahasa') }}
                    </label>
                    <div class="custom-select" @click.outside="langOpen = false">
                        <input type="hidden" name="language" x-model="language" id="language-input">
                        <button type="button" @click="langOpen = !langOpen" class="custom-select-btn" :class="langOpen && 'open'" id="language-select">
                            <span x-text="language === 'id' ? '🇮🇩 {{ __('Bahasa Indonesia') }}' : '🇬🇧 {{ __('English') }}'"></span>
                            <i class="fa-solid fa-chevron-down text-sm text-slate-400" :class="langOpen && 'rotate-180'" style="transition: transform 0.3s;"></i>
                        </button>
                        <div x-show="langOpen" x-cloak class="custom-select-menu">
                            <button type="button" @click="language = 'id'; langOpen = false;" class="custom-select-item" :class="language === 'id' && 'active'">
                                🇮🇩 {{ __('Bahasa Indonesia') }}
                            </button>
                            <button type="button" @click="language = 'en'; langOpen = false;" class="custom-select-item" :class="language === 'en' && 'active'">
                                🇬🇧 {{ __('English') }}
                            </button>
                        </div>
                    </div>
                    <p class="text-xs text-slate-400 mt-2">{{ __('Perubahan diterapkan setelah halaman dimuat ulang') }}</p>
                </div>
