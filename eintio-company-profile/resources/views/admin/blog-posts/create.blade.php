...
<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-xs text-slate-400 mb-1.5">Beranda / Blog & Artikel / Artikel / Tambah</p>
            <h2 class="text-xl font-bold text-brand-dark tracking-tight">Tambah Artikel</h2>
        </div>
    ...
    <div x-data="articleForm()" class="space-y-6 w-full">
        ...
        <form method="POST" action="{{ route('admin.blog-posts.store') }}" enctype="multipart/form-data" x-ref="mainForm">
            @csrf
            ...
            <div class="reveal form-modal-card bg-white shadow-sm border border-slate-100 px-8 py-7">
                ...
                <div class="flex items-center justify-between mb-6 flex-wrap gap-4 pb-5 border-b border-slate-100">
                    <div>
                        <h3 class="font-bold text-brand-dark text-lg">Tambah Artikel</h3>
                        <p class="text-sm text-slate-400 mt-1">Buat artikel baru untuk ditampilkan di website.</p>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" @click="submitForm('publish')"
                            ...
                            <span class="btn-label">Publikasikan</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-brand-dark mb-2">Tanggal Publish</label>
                            <div class="space-y-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="publish_type" value="now" x-model="publishType"
                                           class="h-4 w-4 text-brand-mint focus:ring-brand-mint border-slate-300">
                                    <span class="text-sm text-slate-600">Publikasikan sekarang</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="publish_type" value="schedule" x-model="publishType"
                                           class="h-4 w-4 text-brand-mint focus:ring-brand-mint border-slate-300">
                                    <span class="text-sm text-slate-600">Jadwalkan</span>
                                </label>
                                <div x-show="publishType === 'schedule'" x-cloak class="field-ring flex items-center px-4 mt-2">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-medium text-brand-dark mb-2">Gambar Cover <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <div class="field-ring px-4">
                                <textarea name="excerpt" x-model="excerpt" maxlength="200" rows="6"
                                          ...
                                          class="w-full border-0 focus:ring-0 text-sm py-2.5 bg-transparent resize-none">{{ old('excerpt') }}</textarea>
                            </div>
                ...
                <template x-if="activeSectionIndex !== null">
                    <div class="modal-panel section-modal-panel relative bg-white rounded-2xl shadow-2xl p-7 w-full max-w-lg z-10">
                        ...
                        <div class="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
                            ...
                            {{-- ===== TYPE dengan items: list / columns / benefits / timeline ===== --}}
    ...
    <script>
        function articleForm() {
            return {
                ...
                excerpt: '{{ old('excerpt') }}',
                featured: {{ old('featured') ? 'true' : 'false' }},
                publishType: 'now',
                action: 'draft',
                thumbPreview: null,
                ...
                submitForm(actionType) {
                    ...
                    if (!this.thumbPreview) { alert('Gambar cover wajib diupload.'); return; }
