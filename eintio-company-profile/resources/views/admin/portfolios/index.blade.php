<x-app-layout>
    ...
    <div x-data="portfolioIndex()" class="space-y-6">
        ...
        <div class="reveal card-hover bg-white rounded-2xl shadow-sm border border-slate-100 overflow-visible" style="transition-delay:.05s">
            <form method="GET" class="flex flex-wrap items-center gap-3 px-6 py-5 border-b border-slate-100 relative z-10">
                ...
                <div class="dropdown-wrap w-52" @click.outside="categoryOpen = false">
                    ...
                    <div x-show="categoryOpen" x-cloak
                        ...
                        <div class="dropdown-list">
                            <button type="button" @click="categoryId = ''; categoryOpen = false; $el.closest('form').submit()"
                                    class="dropdown-option flex items-center justify-between px-3.5 py-2.5 text-sm"
                                    :class="categoryId === '' ? 'is-selected' : ''">
                                Semua Kategori
                            </button>
                            @foreach($categories as $cat)
                                <button type="button" @click="categoryId = '{{ $cat->id }}'; categoryOpen = false; $el.closest('form').submit()"
                                        class="dropdown-option flex items-center justify-between px-3.5 py-2.5 text-sm"
                ...
                <div class="dropdown-wrap w-44" @click.outside="statusOpen = false">
                    ...
                    <div x-show="statusOpen" x-cloak
                        ...
                        <div class="dropdown-list">
                            <button type="button" @click="statusVal = ''; statusOpen = false; $el.closest('form').submit()"
                                    class="dropdown-option px-3.5 py-2.5 text-sm" :class="statusVal === '' ? 'is-selected' : ''">
                                Semua Status
                            </button>
                            <button type="button" @click="statusVal = 'draft'; statusOpen = false; $el.closest('form').submit()"
                                    class="dropdown-option px-3.5 py-2.5 text-sm" :class="statusVal === 'draft' ? 'is-selected' : ''">
                                Draft
                            </button>
                            <button type="button" @click="statusVal = 'published'; statusOpen = false; $el.closest('form').submit()"
                                    class="dropdown-option px-3.5 py-2.5 text-sm" :class="statusVal === 'published' ? 'is-selected' : ''">
                                Published
                            </button>
                        </div>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm table-fixed border-collapse">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-slate-400 text-sm">Belum ada portfolio.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
