...
<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-xs text-slate-400 mb-1.5">Beranda / Tim / Edit</p>
    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('admin.teams.update', $teams) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="reveal card-hover flex items-center justify-between bg-white rounded-2xl shadow-sm border border-slate-100 px-8 py-7">
                <div>
                    <h3 class="text-lg font-bold text-brand-dark">Edit Anggota</h3>
                ...
                <div class="reveal card-hover bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-7" style="transition-delay:.05s">
                    <div>
                        ...
                        <div class="field-ring flex items-center gap-3 px-4">
                            ...
                            <input type="text" name="name" value="{{ old('name', $teams->name) }}" placeholder="Masukkan nama lengkap" required
                    ...
                    <div>
                        ...
                        <div class="field-ring flex items-center gap-3 px-4">
                            ...
                            <input type="text" name="position" value="{{ old('position', $teams->position) }}" placeholder="Contoh: Chief Executive Officer" required
                                   class="w-full bg-transparent border-0 focus:ring-0 text-sm py-3">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="field-ring flex items-start gap-3 px-4 py-3">
                            ...
                            <textarea name="bio" rows="3" placeholder="Deskripsi singkat anggota tim"
                                      class="w-full bg-transparent border-0 focus:ring-0 text-sm resize-none">{{ old('bio', $teams->bio) }}</textarea>
                        </div>
                    ...
                    <div>
                        ...
                        <label class="upload-box relative flex flex-col items-center justify-center h-40 cursor-pointer overflow-hidden bg-brand-cream/40">
                            <template x-if="!photoPreview && !'{{ $teams->photo }}'">
                            ...
                            <img :src="photoPreview || '{{ $teams->photo ? asset('storage/'.$teams->photo) : '' }}'"
                                 x-show="photoPreview || '{{ $teams->photo }}'"
                                 class="max-h-full max-w-full object-contain p-3">
                            <input type="file" name="photo" accept="image/*" class="hidden"
                                   @change="photoPreview = URL.createObjectURL($event.target.files[0])">
                        </label>
                        <p class="text-[11px] text-slate-300 mt-2">Kosongkan jika tidak ingin mengganti foto.</p>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="field-ring flex items-center gap-2 px-3">
                                    ...
                                    <input type="url" name="linkedin" value="{{ old('linkedin', $teams->linkedin) }}" placeholder="https://linkedin.com/in/username"
                            ...
                            <div>
                                ...
                                <div class="field-ring flex items-center gap-2 px-3">
                                    ...
                                    <input type="url" name="instagram" value="{{ old('instagram', $teams->instagram) }}" placeholder="https://instagram.com/username"
                                           class="w-full bg-transparent border-0 focus:ring-0 text-xs py-2.5">
                                </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="field-ring flex items-center gap-2 px-3">
                                    ...
                                    <input type="number" name="order" value="{{ old('order', $teams->order) }}" min="0"
    ...
    <script>
        function teamsForm() {
            return {
                photoPreview: null,
                divisionId: '{{ old('division_id', $teams->division_id) }}',
                divisionOpen: false,
                status: '{{ old('is_active', $teams->is_active ? "1" : "0") }}',
                statusOpen: false,
