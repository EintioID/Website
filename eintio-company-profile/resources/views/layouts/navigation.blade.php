<aside x-data="{ open: false }" class="w-64 bg-gradient-to-b from-brand-navy to-brand-dark flex flex-col justify-between shrink-0 hidden sm:flex">
    ...
    <div class="px-4 py-4 border-t border-white/10">
        <a href="{{ route('admin.account.index') }}" class="flex items-center gap-3 mb-3 group cursor-pointer">
            <div class="relative">
                @if (!empty(Auth::user()->photo))
                    <img src="{{ Storage::url(Auth::user()->photo) }}"
                         ...
                         class="h-9 w-9 rounded-full object-cover
                                ...
                                group-hover:ring-brand-gold group-hover:ring-offset-2 group-hover:ring-offset-brand-dark group-hover:scale-105">
                @else
            ...
            <div>
                <p class="text-white text-sm font-medium leading-none transition-colors duration-200 ease-smooth group-hover:text-brand-gold">{{ Auth::user()->name }}</p>
                <p class="text-brand-beige/60 text-xs mt-1">{{ ucfirst(Auth::user()->role ?? 'Admin') }} • Online</p>
            </div>
        </a>
